...
window.loadTelemetryTab = function() {
    // Optionally: Request calibration over WebSocket here, if available
    if (window.websocketCarInput && websocketCarInput.readyState === WebSocket.OPEN) {
        websocketCarInput.send("IMU,REQUEST_CALIB");
    }
    // Then load calibration data/UI
    if (typeof loadCalibrationData === "function") loadCalibrationData();
};
