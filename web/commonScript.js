...
let controlsPanelReflowRaf = null;
...
function initCollapsibleControlsPanel() {
  ...
  controlsPanelElement.addEventListener('transitionstart', (e) => {
    if (e && e.propertyName === 'left') {
      startControlsPanelReflowLoop(420);
    }
  });
  controlsPanelElement.addEventListener('transitionend', (e) => {
    if (e && e.propertyName === 'left') {
      stopControlsPanelReflowLoop();
...
function refreshControlsPanelPosition() {
  ...
  applyPanelAvoidanceLayout(false);
  if (memoryUiState.open) applyMemoryExpansionLift();
}

function stopControlsPanelReflowLoop() {
  if (controlsPanelReflowRaf === null) {
    return;
  }
  if (typeof window.cancelAnimationFrame === 'function') {
    window.cancelAnimationFrame(controlsPanelReflowRaf);
  } else {
    clearTimeout(controlsPanelReflowRaf);
  }
  controlsPanelReflowRaf = null;
}

function startControlsPanelReflowLoop(durationMs) {
  stopControlsPanelReflowLoop();
  const duration = Math.max(200, Number(durationMs) || 420);
  ...
  const tick = () => {
    ...
    if (now - start < duration) {
      if (typeof window.requestAnimationFrame === 'function') {
        controlsPanelReflowRaf = window.requestAnimationFrame(tick);
      } else {
        controlsPanelReflowRaf = window.setTimeout(tick, 16);
      }
      return;
    }
    controlsPanelReflowRaf = null;
...
function realOnLoad() {
  ...
  fetch('/getsettings')
    ...
    .then(data => {
      ...
      if ("ImuVisible" in data) {
        overlayWidgetStates.imu.visible = (data.ImuVisible == 1);
      }
      if ("ImuX" in data && "ImuY" in data) {
        const imuX = parseInt(data.ImuX, 10);
        const imuY = parseInt(data.ImuY, 10);
        if (!Number.isNaN(imuX) && !Number.isNaN(imuY)) {
          overlayWidgetStates.imu.x = imuX;
          overlayWidgetStates.imu.y = imuY;
        }
      }
      if ("MediaVisible" in data) {
        overlayWidgetStates.media.visible = (data.MediaVisible == 1);
      }
      if ("MediaX" in data && "MediaY" in data) {
        const qx = parseInt(data.MediaX, 10);
        const qy = parseInt(data.MediaY, 10);
        if (!Number.isNaN(qx) && !Number.isNaN(qy)) {
          overlayWidgetStates.media.x = qx;
          overlayWidgetStates.media.y = qy;
        }
      }
      if ("PathVisible" in data) {
        overlayWidgetStates.path.visible = (data.PathVisible == 1);
      }
      if ("PathX" in data && "PathY" in data) {
        const px = parseInt(data.PathX, 10);
        const py = parseInt(data.PathY, 10);
        if (!Number.isNaN(px) && !Number.isNaN(py)) {
          overlayWidgetStates.path.x = px;
          overlayWidgetStates.path.y = py;
        }
      }
      if ("Model3DVisible" in data) {
        overlayWidgetStates.model.visible = (data.Model3DVisible == 1);
      }
      if ("Model3DX" in data && "Model3DY" in data) {
        const mx = parseInt(data.Model3DX, 10);
        const my = parseInt(data.Model3DY, 10);
        if (!Number.isNaN(mx) && !Number.isNaN(my)) {
          overlayWidgetStates.model.x = mx;
          overlayWidgetStates.model.y = my;
        }
      }
      if ("ViewOverlapFx" in data) {
        overlapEffectEnabled = (parseInt(data.ViewOverlapFx, 10) !== 0);
      }
      if ("ViewSnapFx" in data) {
        snapEffectEnabled = (parseInt(data.ViewSnapFx, 10) !== 0);
      }
      if ("ViewGravityFx" in data) {
        gravityEffectEnabled = (parseInt(data.ViewGravityFx, 10) !== 0);
      }
      if ("ViewGravityStr" in data) {
        const gs = parseInt(data.ViewGravityStr, 10);
        if (!Number.isNaN(gs)) gravityEffectStrength = Math.max(0, Math.min(100, gs));
      }
      syncGravityToggleUi();
      applyOverlayWidgetVisibility("imu");
      applyOverlayWidgetPosition("imu", false);
      applyOverlayWidgetVisibility("media");
      applyOverlayWidgetPosition("media", false);
      applyOverlayWidgetVisibility("path");
      applyOverlayWidgetPosition("path", false);
      applyOverlayWidgetVisibility("model");
      applyOverlayWidgetPosition("model", false);
...
function initWebSocket() {
  ...
  websocketCarInput.onmessage = function (event) {
    const deliver = (text) => {
      ...
      if (key === "ImuVisible") {
        overlayWidgetStates.imu.visible = (value == "1");
        applyOverlayWidgetVisibility("imu");
        persistOverlayWidgetState("imu", false);
        refreshViewWidgetPalette();
        return;
      }
      if (key === "ImuX") {
      ...
      if (key === "ImuY") {
        const vy = parseInt(value, 10);
        if (!Number.isNaN(vy)) {
          overlayWidgetStates.imu.y = vy;
          applyOverlayWidgetPosition("imu", false);
          persistOverlayWidgetState("imu", false);
        }
        return;
      ...
      if (key === "MediaVisible") {
        overlayWidgetStates.media.visible = (value == "1");
        applyOverlayWidgetVisibility("media");
        persistOverlayWidgetState("media", false);
        refreshViewWidgetPalette();
        return;
      }
      if (key === "MediaX") {
        const qx = parseInt(value, 10);
        if (!Number.isNaN(qx)) {
          overlayWidgetStates.media.x = qx;
          applyOverlayWidgetPosition("media", false);
          persistOverlayWidgetState("media", false);
      ...
      if (key === "PathVisible") {
        overlayWidgetStates.path.visible = (value == "1");
        applyOverlayWidgetVisibility("path");
        persistOverlayWidgetState("path", false);
        refreshViewWidgetPalette();
        return;
      }
      if (key === "PathX") {
      ...
      if (key === "Model3DVisible") {
        overlayWidgetStates.model.visible = (value == "1");
        applyOverlayWidgetVisibility("model");
        persistOverlayWidgetState("model", false);
        refreshViewWidgetPalette();
        return;
      }
      if (key === "Model3DX") {
        const mx = parseInt(value, 10);
        if (!Number.isNaN(mx)) {
          overlayWidgetStates.model.x = mx;
          applyOverlayWidgetPosition("model", false);
          persistOverlayWidgetState("model", false);
      ...
      if (key === "ViewOverlapFx") {
        overlapEffectEnabled = (value == "1");
        syncGravityToggleUi();
        return;
      }
      if (key === "ViewSnapFx") {
        snapEffectEnabled = (value == "1");
        syncGravityToggleUi();
        return;
      }
      if (key === "ViewGravityFx") {
        gravityEffectEnabled = (value == "1");
        syncGravityToggleUi();
        return;
      }
      if (key === "ViewGravityStr") {
        const gs = parseInt(value, 10);
        if (!Number.isNaN(gs)) {
          gravityEffectStrength = Math.max(0, Math.min(100, gs));
          syncGravityToggleUi();
        }
        return;
      }
