/*
Implemented logging all telemetry in CSV file format
Implemented chart drawing (currently only battery voltage when clicked on overlay)
*/
#include <Arduino.h>
...
#define FIRMWARE_VERSION "v2.0.55"
...
#define PIN_LEFT_MOTOR_IN2  19  //for serial debut to function put these  as -1 and to control motors back to 19
#define PIN_ARM_MOTOR_IN1   43  //for serial debut to function put these  as -1 and to control motors back to 43
#define PIN_ARM_MOTOR_IN2   44  //for serial debut to function put these  as -1 and to control motors back to 44
...
void flushTelemetryBufferToSD_Auto() {
  ...
  // Estimate the size we will append
  size_t estimatedSize = telemetryBuffer.size() * sizeof(TelemetrySample); // rough estimate
  ...
  for (const auto& s : telemetryBuffer) {
      // Write human-readable datetime (if possible), else empty
      if (timeIsValid) {
          ...
          f.print(""); // no valid time, empty cell
      }
      f.print(",");           // CSV separator

      // Then write the rest as before
  ...
  // ---- Reset currentSample for next round ----
  resetCurrentSample();
}
...
void handleControllerInput(ControllerPtr ctl) {
    // === 1. DRIVE CONTROL ===
    int x = ctl->axisX(); // -511 (left) to 511 (right)
    int y = ctl->axisY(); // -511 (up) to 511 (down)

    int inputValue = STOP;
    if (abs(x) < STICK_DEADZONE && abs(y) < STICK_DEADZONE) {
        inputValue = STOP;
    } else if (abs(y) > abs(x)) {
        if (y < -STICK_DEADZONE)      inputValue = UP;
        else if (y > STICK_DEADZONE)  inputValue = DOWN;
    } else {
        if (x < -STICK_DEADZONE)      inputValue = LEFT;
        else if (x > STICK_DEADZONE)  inputValue = RIGHT;
    }
    moveCar(inputValue);

    // === 2. ARM MOVEMENT ===
    if (ctl->l1()) moveCar(ARMUP);
    if (ctl->r1()) moveCar(ARMDOWN);

    // === 3. LIGHT/BEACON/EMERGENCY TOGGLES (rising edge) ===
    ...
    // A button toggles beacon
    if (ctl->a() && !lastA) {
        beaconOn = !beaconOn;
        updatePixels();
        Serial.println(beaconOn ? "🟡 Beacon ON (via gamepad)" : "⚫ Beacon OFF (via gamepad)");
    }
    lastA = ctl->a();

    // Y button toggles emergency
    if (ctl->y() && !lastY) {
        emergencyOn = !emergencyOn;
        updatePixels();
        Serial.println(emergencyOn ? "🟡 Emergency ON (via gamepad)" : "⚪ Emergency OFF (via gamepad)");
    }
    lastY = ctl->y();

    // === 4. TURN SIGNALS using D-pad bitfield ===
    bool dpadLeft = ctl->dpad() & DPAD_LEFT;
    bool dpadRight = ctl->dpad() & DPAD_RIGHT;
    ...
    // === 5. BUCKET/AUX CONTROLS (optional) ===
...
void animateGears() {
  ...
  int r1 = 10, r2 = 15;
  int teeth1 = 8, teeth2 = 12;
  int x1 = 50, y1 = 32;
  int x2 = x1 + r1 + r2 - 1;

  // Load user OLED settings
  ...
  if (oledLayout == "default") {
    ...
    if (f) {
      uint8_t buffer[1024]; // for 128x64 monochrome
...
void displayXBMFromSD(const char* filename) {
  File f = SD.open(filename);
  if (!f) return;

  uint8_t buf[1024]; // enough for 128x64 1-bit (1024 bytes)
  int i = 0;
  while (f.available() && i < sizeof(buf)) {
    buf[i++] = f.read();
  }
  f.close();

  display.clearDisplay();
  display.drawBitmap(0, 0, buf, 128, 64, SSD1306_WHITE);
  display.display();
}
//---------------------------------------------------------------I2C---------------------------------------------------------------------------------------
...
void spiStart() {
    ...
    if (!SD.begin(SD_CS)) {
        ...
        // --- Add telemetry folder here ---
        if (!SD.exists("/telemetry")) {
            SD.mkdir("/telemetry");
            Serial.println("Created /telemetry folder on SD card.");
        }
    }
}
    ...
    case WS_EVT_DATA:
      ...
      if (info->final && info->index == 0 && info->len == len && info->opcode == WS_TEXT) {
        ...
        if (key == "START_MIC_STREAM") {
            ...
            //micCleanupPending = true;   // 🆕 Allow cleanup of GPIO38
        ...
        if (key == "MoveCar") {
            int pwm = (value2.length() == 0) ? 255 : atoi(value2.c_str());
            moveCar(valueInt, pwm);
            Serial.printf("[WS] moveCar called with valueInt=%d pwm=%d\n", valueInt, pwm);
        }
          ...
          horScreen = (valueInt != 0);;
          uiPrefs.putBool("Switch", horScreen);
        }
          ...
          tlmEnabled = (valueInt != 0);
          uiPrefs.putBool("RecordTelemetry", tlmEnabled);
          Serial.printf("RecordTelemetry set to %d\n", tlmEnabled ? 1 : 0);
        }
...
void cleanupMic() {
    if (micCleanupPending) {
        Serial.println("[MIC] Cleaning up mic I2S...");

        // Uninstall I2S driver to release GPIO38
        i2s_driver_uninstall(MY_I2S_PORT);

        // Set GPIO38 back to output LOW (motor standby, etc.)
        //pinMode(38, OUTPUT);
        //digitalWrite(38, LOW);

        currentI2SMode = I2S_NONE;
        micCleanupPending = false;
        Serial.println("[MIC] Mic disabled, GPIO38 released and pulled LOW.");
    }
}
...
void serverStart(){
  ...
  server.on("/getsettings", HTTP_GET, [](AsyncWebServerRequest *request){
    ...
    json += ",\"horizontalScreen\":" + String(horScreen ? 1 : 0);
    json += ",\"recordTelemetry\":" + String(tlmEnabled ? 1 : 0);
...
server.onNotFound([](AsyncWebServerRequest *request) {
    ...
    // ---- Custom: handle /telemetry/* static CSV files ----
  ...
  server.on("/disable_mic", HTTP_GET, [](AsyncWebServerRequest *request) {
      ...
      //micCleanupPending = true;        // Cleanup in loop()
  ...
  server.on("/get_oled_settings", HTTP_GET, [](AsyncWebServerRequest *request) {
    String layout = oledPrefs.getString("layout", "default");
    bool showIP = oledPrefs.getBool("showIP", true);
    bool showBattery = oledPrefs.getBool("showBattery", true);
    bool showWiFi = oledPrefs.getBool("showWiFi", true);
    ...
    String json = "{";
    json += "\"layout\":\"" + layout + "\",";
    json += "\"showIP\":" + String(showIP ? "true" : "false") + ",";
    json += "\"showBattery\":" + String(showBattery ? "true" : "false") + ",";
    json += "\"showWiFi\":" + String(showWiFi ? "true" : "false");
    json += "}";

    request->send(200, "application/json", json);
  });

  server.on("/set_oled_settings", HTTP_POST, [](AsyncWebServerRequest *request){}, NULL,
  [](AsyncWebServerRequest *request, uint8_t *data, size_t len, size_t index, size_t total) {
    ...
    String layout = doc["layout"] | "default";
    bool showIP = doc["showIP"] | true;
    bool showBattery = doc["showBattery"] | true;
    bool showWiFi = doc["showWiFi"] | true;
  ...
  server.on("/upload_oled_anim", HTTP_POST, [](AsyncWebServerRequest *request) {
    request->send(200, "text/plain", "OK");
  }, [](AsyncWebServerRequest *request, String filename, size_t index, uint8_t *data,
        size_t len, bool final) {
    static File uploadFile;
    if (!index) {
      Serial.printf("📤 Uploading OLED animation file: %s\n", filename.c_str());
    ...
    if (uploadFile) {
      ...
      if (final) {
        uploadFile.close();
        Serial.println("✅ OLED animation upload complete.");
      }
    }
  });
      ...
      if (dir && dir.isDirectory()) {
          ...
          while (f) {
              ...
              if (name.endsWith(".csv")) {
                  if (!first) result += ",";
                  // Ensure the name is the full path
...
void sendBatteryTelemetryIfIdle() {
  ...
  if (!audio.isRunning() && millis() - lastTelemetrySend > 1000) {
    ...
    const float correctionFactorB = 8.4 / 10; // tune if needed voltage taken by multimeter divided by actual reading
    ...
    currentSample.voltage = batteryVoltage;
    currentSample.temp    = chipTemp;
    currentSample.charger = chargerVoltage;
...
void setup() {
  ...
  */
  ...
  /*
    ...
    BP32.enableNewBluetoothConnections(true);  // Allow pairing
...
void loop() {
  ...
  sendImuTelemetry();
  sendFpsTelemetry();
  flushTelemetryBufferToSD_Auto();  //Logging
