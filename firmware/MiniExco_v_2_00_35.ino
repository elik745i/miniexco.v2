...
#define FIRMWARE_VERSION "v2.0.35"
...
void updatePixels() {
  ...
  if (emergencyOn) {
    // All 4 turn signals flash yellow
    uint32_t col = (millis() / 400) % 2 ? pixels.Color(255, 180, 0) : 0;
    pixels.setPixelColor(0, col);    // Pixel 1
    pixels.setPixelColor(5, col);    // Pixel 6
    pixels.setPixelColor(6, col);    // Pixel 7
    pixels.setPixelColor(11, col);   // Pixel 12
    pixels.show();
    return;
  }

  // Next: Beacon (police mode)
  if (beaconOn) {
  ...
  // Next: Turn signals (but only one side at a time)
...
void processReindexTask() {
    ...
    int batchCount = 0;
    const int BATCH_SIZE = 50; // files per loop, adjust as needed
    while (f && batchCount < BATCH_SIZE) {
        String name = String(f.name());
        // Skip .index and dot-files!
        if (name.startsWith(".") || name == ".index") { f.close(); f = dir.openNextFile(); continue; }
        if (name.equalsIgnoreCase("System Volume Information") || name == "Thumbs.db") { f.close(); f = dir.openNextFile(); continue; }
        idx.printf("%s,%d,%u\n", name.c_str(), f.isDirectory() ? 1 : 0, (unsigned)f.size());
        f.close();
        f = dir.openNextFile();
        batchCount++;
        reindexCount++;
...
void readSdIndexBatch(const String& idxPath, int start, int count, JsonArray& arr, bool showSystem = false) {
    ...
    while (idx.available()) {
        ...
        // ... your file parsing/filtering logic ...
        ...
        uint32_t size = line.substring(comma2 + 1).toInt();
  ...
  switch (type) {
    ...
    case WS_EVT_DATA:
      ...
      if (info->final && info->index == 0 && info->len == len && info->opcode == WS_TEXT) {
        ...
        // Add to onCarInputWebSocketEvent, inside WS_EVT_DATA:
        if (key == "MEDIA_PLAY") {
...
void playWavFileOnSpeaker(const String& filename) {
    ...
    audio.stopSong();
    // Start the file (works for .wav, .mp3, .ogg)
...
void serverStart(){
  ...
  server.on("/list_media_files", HTTP_GET, [](AsyncWebServerRequest *request) {
      ...
      readSdIndexBatch(indexPath, start, count, arr, false);  // you can optionally add filter, but at least count here!

      doc["start"] = start;
      doc["count"] = arr.size();

      // === Add this line! ===
      doc["total"] = countMediaFilesInIndex(indexPath);
  ...
  server.on("/play_on_device", HTTP_GET, [](AsyncWebServerRequest *request){
      ...
      if (!file.startsWith("/media/")) file = "/media/" + file;
      if (!SD.exists(file)) {
          request->send(404, "text/plain", "File not found");
          return;
      }

      // Optionally, do this async/task or set a flag!
  ...
  if (wifiConnected) {
      ...
      struct tm tm;
      // Wait until time is actually set (NTP sync happens in background)
      int waitSec = 0;
      while (!getLocalTime(&tm) && waitSec < 10) { // Try up to 10 sec
          delay(500);
          waitSec += 0.5;
      ...
      if(getLocalTime(&tm)) {
          ...
          Serial.println("Time not set after waiting!");
      }
...
void setup() {
  ...
  audio.setPinout(I2S_SPK_BCLK, I2S_SPK_LRCK, I2S_SPK_SD);
  audio.setVolume(15); // 0-21, change as needed

  pinMode(I2S_SPK_PA, OUTPUT);      // Keep this
  digitalWrite(I2S_SPK_PA, HIGH);   // Keep this
...
void loop() {
  ...
  // ---------- TELEMETRY: BATTERY, WIFI, TEMP, UPTIME (1s) ----------
  ...
  if (millis() - lastTelemetrySend > 1000) {
    ...
    const float MIN_BATTERY_VOLTAGE = 6.6;   // set for your hardware!
  ...
  // ---------- FPS COUNTING (increment frameCount++ in stream handler!) ----------
  static unsigned long lastFpsSend = 0;
  extern volatile int frameCount;  // define volatile int frameCount = 0; globally!
  ...
  // In your stream handler: frameCount++;

  // ---------- MOTOR/PATH LOGIC: (insert your logic here) ----------
  // [Insert non-blocking motor ramp/path following code]

  // ---------- SERVO DETACH ----------
  if (!holdAux && auxAttached && millis() > auxDetachTime) {
    ledcDetachPin(auxServoPin); auxAttached = false;
  }
  if (!holdBucket && bucketAttached && millis() > bucketDetachTime) {
    ledcDetachPin(bucketServoPin); bucketAttached = false;
  }
  ...
  const unsigned long WIFI_SCAN_INTERVAL = 20000; // 20s, tweak as needed
/*
  if (WiFi.getMode() == WIFI_AP && millis() - lastWiFiScan > WIFI_SCAN_INTERVAL) {
    lastWiFiScan = millis();
    int n = WiFi.scanNetworks();
    String savedList = wifiPrefs.getString("networks", "");
    int lastIndex = 0, nextIndex;
    ...
    while ((nextIndex = savedList.indexOf(',', lastIndex)) != -1) {
      ...
      for (int i = 0; i < n; i++) {
        if (ssid == WiFi.SSID(i)) {
          Serial.println("🔁 Auto-trying SSID: " + ssid);
          String password = wifiPrefs.getString(("wifi_" + ssid).c_str(), "");
          connected = connectToWiFiWithRetries(ssid, password, 3);
          if (connected) break;
        }
      }
      if (connected) break;
    }
    // Last SSID in the list
    if (!connected && lastIndex < savedList.length()) {
      String ssid = normalizedSSID(savedList.substring(lastIndex));
      bool autoReconnect = wifiPrefs.getBool(("aRt_" + ssid).c_str(), true);
      if (autoReconnect) {
        for (int i = 0; i < n; i++) {
...
*/
  // Update beacon phase for police pattern every 180 ms if active
  if (beaconOn && millis() - lastAnimUpdate > 180) {
    beaconPhase = (beaconPhase + 1) % 4;
    lastAnimUpdate = millis();
    updatePixels();
  } else if (emergencyOn || leftSignalActive || rightSignalActive || light) {
    updatePixels();
  }
  ...
  // --- Wi-Fi State Machine Monitor ---
  if (millis() - wifiLastCheck > 1000) {  // Check every 1s
    ...
    if (wifiState == WIFI_STA_RETRYING) {
      if (wifiAutoRetryCount < 3) {
        ...
        wifiState = WIFI_AP_MODE;
        wifiOledLastStep = "";
        wifiRetryStart = millis(); // For next scan in AP mode
      }
    }

    // 4. In AP mode, scan for saved networks every 15s
    if (wifiState == WIFI_AP_MODE && millis() - wifiRetryStart > 15000) {
      ...
      if (found && foundSSID.length()) {
        ...
        wifiRetryStart = millis(); // try again in 15s
      }
    }
  }
